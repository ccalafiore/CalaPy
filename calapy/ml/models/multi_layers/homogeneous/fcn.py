...
__all__ = ['SequentialFCLs', 'ParallelFCLs', 'SequentialFCLsParallelFCLs']
...
class SequentialFCLs(cp_ModelMethods):
    ...
    def __init__(
        ...
        self.layers = torch.nn.Sequential(*[torch.nn.Linear(
            ...
            bias=self.biases_layers[l - 1], device=device) for l in range(1, self.L, 1)])
...
class ParallelFCLs(cp_ModelMethods):
    ...
    def __init__(
        ...
        self.layers = torch.nn.ModuleList([torch.nn.Sequential(
            *[torch.nn.Linear(
                ...
                bias=self.biases_layers[m][l - 1], device=device)
...
class SequentialFCLsParallelFCLs(cp_ModelMethods):
    ...
    def __init__(
        ...
        superclass = SequentialFCLsParallelFCLs
        try:
            # noinspection PyUnresolvedReferences
            self.superclasses_initiated
        except AttributeError:
